...
export function calculateMinimumSeverity(
  ...
  if (injuryClassification && (SEVERITY_VALIDATION_TRIGGERS.fatalityRequired as readonly string[]).includes(injuryClassification)) {
    return {
      minLevel: 'level_5',
  ...
  if (injuryClassification && (SEVERITY_VALIDATION_TRIGGERS.ltiMinimum as readonly string[]).includes(injuryClassification)) {
    return {
      minLevel: 'level_4',
  ...
  if (erpActivated || (eventType && (ERP_EVENT_TYPES as readonly string[]).includes(eventType))) {
    return {
      minLevel: 'level_4',
