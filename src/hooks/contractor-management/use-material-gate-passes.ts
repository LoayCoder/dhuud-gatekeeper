...
interface BulkApproveParams {
  passIds: string[];
  approvalType?: "pm" | "safety"; // Deprecated, but kept for interface compatibility
  notes?: string;
}
  ...
  return useMutation({
    mutationFn: async ({ passIds, notes }: BulkApproveParams): Promise<BulkResult> => {
      ...
      for (const passId of passIds) {
        try {
          // Use the unified RPC for approval
          // We don't need to manually check statuses or set next status, the RPC does it.
          const { error } = await supabase.rpc("approve_gate_pass_unified", {
            p_user_id: user.id,
            p_gate_pass_id: passId,
            p_action: "approve",
            p_notes: notes || null,
