    ...
    <Card>
      <CardHeader className="pb-2 sm:pb-3">
        <CardTitle className="text-sm sm:text-base flex items-center gap-2">
          ...
          <span className="truncate">{t('security.gateDashboard.activeWorkers', 'Active Workers')}</span>
          <Badge variant="secondary" className="ms-auto flex-shrink-0 text-xs">
      ...
      <CardContent className="px-3 sm:px-6">
        {activeWorkers.length === 0 ? (
          <div className="text-center py-6 sm:py-8 text-muted-foreground">
            <HardHat className="h-6 w-6 sm:h-8 sm:w-8 mx-auto mb-2 opacity-50" />
            <p className="text-xs sm:text-sm">{t('security.gateDashboard.noActiveWorkers', 'No active workers on site')}</p>
          </div>
        ) : (
          <ScrollArea className="h-[220px] sm:h-[280px] md:h-[300px] pe-2 sm:pe-4">
            <div className="space-y-2 sm:space-y-3">
              {activeWorkers.map((worker) => (
                ...
                >
                  <Avatar className="h-8 w-8 sm:h-10 sm:w-10 flex-shrink-0">
                    <AvatarFallback className="text-xs sm:text-sm bg-amber-500/10 text-amber-600 dark:text-amber-400">
                      {worker.person_name?.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() || '?'}
                    </AvatarFallback>
                  </Avatar>
                  
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm sm:text-base truncate">{worker.person_name || 'Unknown'}</p>
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] sm:text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Clock className="h-2.5 w-2.5 sm:h-3 sm:w-3 flex-shrink-0" />
                        {format(new Date(worker.entry_time), 'HH:mm')}
                      </span>
                  ...
                  >
                    <LogOut className="h-3 w-3 sm:me-1" />
                    <span className="hidden sm:inline text-xs">{t('security.gateDashboard.exit', 'Exit')}</span>
                  </Button>
                </div>
              ))}
            </div>
