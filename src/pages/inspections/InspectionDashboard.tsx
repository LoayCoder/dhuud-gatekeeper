                ...
                {recentFindings.map((finding) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <StatusBadge 
                        status={getClassificationStatus(finding.classification) as "warning" | "destructive" | "success" | "info"}
                        size="sm"
                      >
                        {t(`inspectionDashboard.classifications.${finding.classification}`)}
                      </StatusBadge>
                      <Button variant="ghost" size="sm" asChild>
                        <Link to={`/inspections/sessions/area/${(finding as Record<string, unknown> & { session?: { id?: string } }).session?.id || ''}`}>
                          {t('common.view')}
                        </Link>
                      </Button>
                    </div>
