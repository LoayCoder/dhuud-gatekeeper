...
export default function ContractorAccess() {
  ...
  const handleVerifyWorker = async () => {
    ...
    try {
      // Call the validate-worker-qr edge function
      const { data, error } = await supabase.functions.invoke('validate-worker-qr', {
        body: { qr_data: searchQuery, search_mode: true }
      });
      ...
      if (data?.valid) {
        ...
        setVerificationResult({
          status: 'denied',
          message: data?.error || t('security.contractorAccess.workerNotFound', 'Worker not found or access denied')
        });
      }
    } catch (error) {
  ...
  const handleLogEntry = async () => {
    ...
    try {
      const { error } = await supabase.from('contractor_access_logs').insert({
        contractor_id: verificationResult.worker.id,
        access_type: 'entry',
        validation_status: verificationResult.status === 'granted' ? 'valid' : 'warning',
        tenant_id: verificationResult.worker.tenant_id
      });
  ...
  const handleRecordExit = async () => {
    ...
    try {
      // Find existing entry log and update exit time
      const { data: existingLog, error: fetchError } = await supabase
        .from('contractor_access_logs')
        .select('id')
        .eq('contractor_id', verificationResult.worker.id)
        .is('exit_time', null)
      ...
      if (existingLog) {
        ...
        // Log exit directly if no entry found
        const { error } = await supabase.from('contractor_access_logs').insert({
          contractor_id: verificationResult.worker.id,
          access_type: 'exit',
                      ...
                      <div className="flex-1 space-y-2">
                        ...
                        {verificationResult.worker && (
                          <div className="grid grid-cols-2 gap-2 text-sm">
                            ...
                            <div>
                              ...
                              <span className="ms-2 font-medium">{verificationResult.worker.company?.company_name}</span>
                            </div>
                            <div>
                        ...
                        {verificationResult.worker && verificationResult.status !== 'denied' && (
                          <div className="flex gap-2 pt-2">
                            <Button onClick={handleLogEntry} className="bg-green-600 hover:bg-green-700">
                              <CheckCircle2 className="h-4 w-4 me-2" />
                              {t('security.contractorAccess.logEntry', 'Log Entry')}
                            </Button>
                            <Button variant="outline" onClick={handleRecordExit}>
