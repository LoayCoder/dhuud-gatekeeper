...
export function useFindingsTrend(filters: AnalyticsFilters) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let filtered = data || [];
      if (filters.siteId) {
        filtered = filtered.filter((f: FindingData) => f.session?.site_id === filters.siteId);
      }

      // Group by date and classification
      const byDate: Record<string, Record<string, number>> = {};
      filtered.forEach((finding: FindingData) => {
