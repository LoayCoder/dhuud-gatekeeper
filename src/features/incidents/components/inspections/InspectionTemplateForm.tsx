...
import { InspectionTemplate } from '@/features/incidents';
import { useInspectionTemplateCategories, AREA_TYPES, TEMPLATE_TYPES } from '@/features/incidents';
  ...
  useEffect(() => {
    if (open) {
      form.reset({
        ...
        inspection_category_id: (template as unknown)?.inspection_category_id || undefined,
        area_type: (template as unknown)?.area_type || undefined,
        standard_reference: (template as unknown)?.standard_reference || '',
        passing_score_percentage: (template as unknown)?.passing_score_percentage || 80,
        estimated_duration_minutes: (template as unknown)?.estimated_duration_minutes || undefined,
        requires_photos: (template as unknown)?.requires_photos || false,
        requires_gps: (template as unknown)?.requires_gps || false,
