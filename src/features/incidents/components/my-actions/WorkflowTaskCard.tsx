...
import { formatFallbackLabel } from '@/pages/incidents/MyActions/helpers';
import type {
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {incident?.severity_v2 && (
              <Badge variant={incident.severity_v2.includes('4') || incident.severity_v2.includes('5') ? 'destructive' : 'outline'}>
                {String(t(`incidents.severity.${incident.severity_v2}`, formatFallbackLabel(incident.severity_v2)))}
              </Badge>
            )}
            {isOverdue && (
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {assessment.status && (
              <Badge variant={getStatusVariant(assessment.status)}>
                {String(t(`riskAssessment.status.${assessment.status}`, formatFallbackLabel(assessment.status)))}
              </Badge>
            )}
            {assessment.overall_risk_rating && (
              <Badge variant={assessment.overall_risk_rating === 'high' ? 'destructive' : 'outline'}>
                {String(t(`riskAssessment.rating.${assessment.overall_risk_rating}`, formatFallbackLabel(assessment.overall_risk_rating)))}
              </Badge>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {permit.status && (
              <Badge variant={getStatusVariant(permit.status)}>
                {String(t(`ptw.status.${permit.status}`, formatFallbackLabel(permit.status)))}
              </Badge>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {request.status && (
              <Badge variant={getStatusVariant(request.status)}>
                {String(t(`visitors.status.${request.status}`, formatFallbackLabel(request.status)))}
              </Badge>
            )}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
          ...
          {schedule.frequency_type && (
            <div>
              ...
              {String(t(`inspections.frequency.${schedule.frequency_type}`, formatFallbackLabel(schedule.frequency_type)))}
            </div>
          )}
        </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {gatePass.status && (
              <Badge variant={getStatusVariant(gatePass.status)}>
                {String(t(`gatePasses.status.${gatePass.status}`, formatFallbackLabel(gatePass.status)))}
              </Badge>
            )}
            {gatePass.pass_type && (
              <Badge variant="outline">
                {String(t(`gatePasses.type.${gatePass.pass_type}`, formatFallbackLabel(gatePass.pass_type)))}
              </Badge>
            )}
          </div>
