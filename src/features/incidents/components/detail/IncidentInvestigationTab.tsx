...
export function IncidentInvestigationTab({ incidentId }: IncidentInvestigationTabProps) {
    ...
    const { data: investigation, isLoading } = useQuery<InvestigationDetail | null>({
        ...
        `)
                ...
                .maybeSingle();
            return data as unknown as InvestigationDetail | null;
        },
        enabled: !!incidentId
    });

    const { data: rca } = useQuery<RCADetail | null>({
        ...
        queryFn: async () => {
            const { data } = await supabase
                ...
                .eq('incident_id', incidentId)
                .maybeSingle();
            return data as unknown as RCADetail | null;
        },
        enabled: !!incidentId
    });
