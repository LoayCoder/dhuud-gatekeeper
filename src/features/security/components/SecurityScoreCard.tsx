...
import { useSecurityScore } from '@/features/security';
...
export function SecurityScoreCard({ compact = false, className }: SecurityScoreCardProps) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-muted-foreground">
                  {isRTL ? 'Ø¯Ø±Ø¬Ø© Ø§Ù„Ø£Ù…Ø§Ù†' : 'Security Score'}
                </p>
  ...
  const metricItems = [
    {
      label: isRTL ? 'ØªØ·Ø¨ÙŠÙ‚ Ø§Ù„Ù‚Ø§Ø¦Ù…Ø© Ø§Ù„Ø³ÙˆØ¯Ø§Ø¡' : 'Blacklist Enforcement',
      value: metrics.blacklistEnforcement,
      icon: Ban,
    },
    {
      label: isRTL ? 'Ø§Ù…ØªØ«Ø§Ù„ ØµÙ„Ø§Ø­ÙŠØ© QR' : 'QR Expiry Compliance',
      value: metrics.qrExpiryCompliance,
      icon: CheckCircle2,
    },
    {
      label: isRTL ? 'Ø³Ø±Ø¹Ø© Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø§Øª' : 'Approval Turnaround',
      value: metrics.approvalTurnaround,
      icon: Clock,
    },
    {
      label: isRTL ? 'Ø§Ø³ØªØ¬Ø§Ø¨Ø© Ø§Ù„Ø·ÙˆØ§Ø±Ø¦' : 'Emergency Response',
        ...
        {metrics.activeEmergencies > 0 && (
          <div className="flex items-center gap-2 p-2 rounded-lg bg-destructive/10 border border-destructive/20">
            ...
            <span className="text-sm text-destructive font-medium">
              {isRTL 
                ? `${metrics.activeEmergencies} Ø­Ø§Ù„Ø§Øª Ø·ÙˆØ§Ø±Ø¦ Ù†Ø´Ø·Ø©`
