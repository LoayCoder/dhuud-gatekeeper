...
export async function getGateEntries(tenantId: string, filters?: GateEntryFilters) {
    let query = supabase
...
export type CreateGateEntryParams = Omit<GateEntryInsert, 'tenant_id' | 'guard_id'> & {
    host_mobile?: string;
    notify_host?: boolean;
...
export async function createGateEntry(entry: CreateGateEntryParams, tenantId: string, guardId: string | undefined) {
...
export interface GateWhatsAppParams {
    entryId: string;
    visitorName: string;
    phoneNumber: string;
    hostName?: string;
...
export async function sendGateWhatsAppNotification(params: GateWhatsAppParams) {
