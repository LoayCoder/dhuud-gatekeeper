...
import { useAssetsWithGPS } from '@/features/assets';
...
export function AssetLocationMap({
  ...
  useEffect(() => {
    (window as unknown).navigateToAsset = (id: string) => navigate(`/assets/${id}`);
    (window as unknown).navigateToEditAsset = (id: string) => navigate(`/assets/register?edit=${id}`);
    return () => {
      delete (window as unknown).navigateToAsset;
      delete (window as unknown).navigateToEditAsset;
    };
  }, [navigate]);
      ...
      if (asset.isInspectionOverdue) {
        alertBadges.push(`
          <span style="
            ...
            font-weight: 500;
          ">
             ${t('assets.inspectionOverdue', 'Inspection Overdue')} (${asset.daysInspectionOverdue}d)
          </span>
        `);
      }
      if (asset.isMaintenanceOverdue) {
        alertBadges.push(`
          <span style="
            ...
            font-weight: 500;
          ">
            ðŸ”§ ${t('assets.maintenanceOverdue', 'Maintenance Overdue')} (${asset.daysMaintenanceOverdue}d)
          </span>
        `);
      }
