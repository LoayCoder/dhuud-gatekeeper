...
export function useInvestigationReadiness(incidentId: string | null) {
  return useQuery({
    ...
    queryFn: async (): Promise<InvestigationReadiness> => {
      ...
      const { data, error } = await (supabase.rpc as unknown as (fn: string, params: Record<string, string>) => Promise<{ data: unknown; error: Error | null }>)('check_investigation_readiness', { p_incident_id: incidentId });
