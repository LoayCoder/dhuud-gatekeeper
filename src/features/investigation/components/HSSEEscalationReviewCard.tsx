...
import { useCanReviewHSSEEscalation, useHSSEEscalationReview } from '@/features/incidents';
...
export function HSSEEscalationReviewCard({ incident, onComplete }: HSSEEscalationReviewCardProps) {
  ...
  const deptRepNotes = vi.dept_rep_notes;
        ...
        <CardContent className="space-y-4">
          ...
          <div className="rounded-lg bg-muted/50 p-4 space-y-2">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              {vi.severity_level && (
                <Badge variant="outline">L{vi.severity_level}</Badge>
              )}
            </div>
