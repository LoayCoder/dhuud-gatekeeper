...
import { useInvestigationCompleteness } from '@/features/investigation';
import { useSubmitInvestigation } from '@/features/investigation';
import { useInvestigationReadiness } from '@/features/investigation';
import { InvestigationReadinessDialog } from './InvestigationReadinessDialog';
...
export function SubmitInvestigationCard({ incidentId, onSubmitted }: SubmitInvestigationCardProps) {
  ...
  const handleSubmit = async () => {
    // 1. Check Readiness (Gate n26) - Server-side validation
    setIsValidating(true);
    try {
      const { data: currentReadiness } = await checkReadiness();
      ...
      <Card className={cn(
        ...
        <CardContent className="space-y-4">
          ...
          <div className="pt-2">
            <Button
              className="w-full"
              disabled={!completeness.isComplete || isSubmitting || isValidating}
