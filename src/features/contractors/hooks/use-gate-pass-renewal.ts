  ...
  return useMutation({
    mutationFn: async (gatePassId: string): Promise<RenewGatePassResult> => {
      ...
      try {
        ...
        throw new Error((error as Error).message || t("contractors.gatePasses.renewalFailed", "Failed to renew gate pass"));
      }
    },
    onSuccess: (result) => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      try {
        ...
        throw new Error((error as Error).message || t("contractors.gatePasses.resubmitFailed", "Failed to resubmit gate pass"));
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["material-gate-passes"] });
