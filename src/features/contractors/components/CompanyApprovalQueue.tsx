        ...
        {pendingCompanies.map((company) => (
          <Card key={company.id} className="border-s-4 border-s-warning">
            ...
            <CardContent className="space-y-4">
              ...
              {company.contractor_site_rep_name && (
                <div className="p-3 rounded-lg bg-muted/50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                    <span>{company.contractor_site_rep_name}</span>
                    {company.contractor_site_rep_email && (
                      <span className="flex items-center gap-1">
                        <Mail className="h-3 w-3" />
                        {company.contractor_site_rep_email}
                      </span>
                    )}
                    {company.contractor_site_rep_phone && (
                      <span className="flex items-center gap-1">
              ...
              {(company.contract_start_date || company.contract_end_date) && (
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <span>
                    ...
                    {company.contract_start_date && format(new Date(company.contract_start_date), "PP")}
                    {company.contract_start_date && company.contract_end_date && " - "}
                    {company.contract_end_date && format(new Date(company.contract_end_date), "PP")}
                  </span>
                </div>
              )}
              ...
              {company.scope_of_work && (
                <div className="text-sm">
                  ...
                  <span className="text-muted-foreground">{company.scope_of_work}</span>
                </div>
              )}
