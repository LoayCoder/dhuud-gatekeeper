...
-- Fix get_hsse_event_dashboard_stats: replace 'pending_closure' with 'pending_review'
CREATE OR REPLACE FUNCTION public.get_hsse_event_dashboard_stats(p_start_date DATE DEFAULT NULL, p_end_date DATE DEFAULT NULL)
RETURNS JSONB
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  WITH summary_stats AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE deleted_at IS NULL AND status = 'pending_review') AS pending_closure,
  ...
  SELECT jsonb_build_object(
    ...
    ) ORDER BY month), '[]'::jsonb) FROM monthly_trend),
...
-- Fix get_top_reporters: change p.department_id to p.assigned_department_id
CREATE OR REPLACE FUNCTION public.get_top_reporters(p_limit INTEGER DEFAULT 10, p_start_date DATE DEFAULT NULL, p_end_date DATE DEFAULT NULL)
RETURNS JSONB
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
