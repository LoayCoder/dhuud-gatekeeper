-- Add email column to profiles table for contact/display purposes
...
-- Drop existing function overloads and recreate with email included
DROP FUNCTION IF EXISTS public.get_users_with_roles_paginated(uuid, text, boolean, uuid, uuid, text, integer, integer);
DROP FUNCTION IF EXISTS public.get_users_with_roles_paginated(uuid, text, boolean, uuid, uuid, text, text, integer, integer);

-- Recreate with email in the result
