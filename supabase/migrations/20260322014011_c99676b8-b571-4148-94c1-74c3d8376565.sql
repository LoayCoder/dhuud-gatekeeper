...
-- Fix: 1) Event type double-counting (security/environmental counted in both 'incident' and specific buckets)
-- Fix: 2) Investigation deleted_at not filtered (soft-deleted investigations still counted)
...
BEGIN
  ...
  -- Event type distribution (mutually exclusive categories - no double counting)
  SELECT jsonb_build_object(
