-- Fix: HSSE Expert should NOT see contractor observations in pending_consultant_screening
-- Root cause: Line 71 allowed any HSSE Expert to approve contractor observations
-- in contractor consultant workflow statuses, bypassing the intended restriction.

DROP FUNCTION IF EXISTS public.can_approve_investigation(uuid, uuid);
...
BEGIN
  ...
  IF v_incident_status IN ('expert_screening', 'pending_consultant_screening', 
    ...
    -- HSSE Expert ONLY for non-contractor observations
    -- FIX: Previously allowed HSSE Expert on contractor observations too
...
COMMENT ON FUNCTION public.can_approve_investigation IS 'Determines if a user can approve/act on an investigation. Contractor Consultants can work on contractor observations they reported (no self-approval restriction for consultant workflow). HSSE Experts are blocked from contractor observations in consultant workflow statuses.';