-- ==============================================================================
-- GATE PASS ENTRY/EXIT RULES UPDATE
-- Purpose: Enforce pass_type-specific date rules for public gate pass requests.
--   - Entry & Exit (in_out): start_date + end_date, up to 7-day range
--   - Entry Only (in): single date only (start_date == end_date)
--   - Exit Only (out): single date only (start_date == end_date)
--   - Remove time_window fields from public submissions (time logged by guards)
-- ==============================================================================
...
-- 4. CREATE FUNCTION TO VALIDATE GATE PASS ACCESS BY GUARD
-- This validates that the guard action matches the pass_type rules
CREATE OR REPLACE FUNCTION public.validate_gate_pass_guard_access(
  p_gate_pass_id UUID,
  p_action TEXT  -- 'entry' or 'exit'
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Validate action against pass_type
  IF v_pass.pass_type = 'in' THEN
    -- Entry Only: only allow entry action
  ...
  ELSIF v_pass.pass_type = 'out' THEN
    -- Exit Only: only allow exit action
  ...
  ELSE
    -- Entry & Exit (in_out): allow both within date range
