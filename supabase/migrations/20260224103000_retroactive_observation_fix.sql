-- Retroactively assigns all stuck/unassigned Contractor-Related Observations 
-- to their respective Contractor Consultants
...
BEGIN
  -- Loop through all contractor-related observations that might be in an unassigned or wrongly assigned screening queue
  FOR v_obs IN 
    ...
    WHERE i.event_type = 'observation' 
      ...
      AND i.status IN ('submitted', 'pending_consultant_screening', 'pending_expert_screening', 'pending_hsse_expert_review')
      AND (i.approval_manager_id IS NULL OR i.status != 'pending_consultant_screening') 
  LOOP
    -- Calculate the effective branch (falling back to the site's branch if the observation itself doesn't have it direct)
    v_effective_branch := COALESCE(v_obs.branch_id, (SELECT branch_id FROM sites WHERE id = v_obs.site_id));
    
    -- Attempt to find an active Contractor Consultant for this specific branch or tenant-wide
    ...
    -- If a specific Consultant is found, assign the observation directly to them
  ...
  RAISE NOTICE 'Successfully retroactively assigned % contractor-related observations to their consultants.', v_updated_count;
END;
$$;
