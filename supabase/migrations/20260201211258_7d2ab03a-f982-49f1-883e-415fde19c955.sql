-- Drop the existing 3-parameter function
DROP FUNCTION IF EXISTS public.can_approve_gate_pass(uuid, uuid, text);

-- Recreate with correct column references and JSONB return type
...
BEGIN
  ...
  -- Get user info
  SELECT p.branch_id, p.department_id
  ...
  CASE p_stage
    ...
    WHEN 'pending_dept_approval' THEN
      -- Department rep or manager can approve if same department
      IF v_is_dept_rep OR v_is_dept_manager THEN
        IF v_user_department_id = v_gate_pass.approval_from_id THEN
          RETURN jsonb_build_object('allowed', true);
        ELSE
          RETURN jsonb_build_object('allowed', false, 'reason', 'Not in the requesting department');
        END IF;
      END IF;
