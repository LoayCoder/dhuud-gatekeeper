-- Add admin override tracking columns to incidents table
ALTER TABLE incidents ADD COLUMN IF NOT EXISTS admin_override_by UUID REFERENCES profiles(id);
ALTER TABLE incidents ADD COLUMN IF NOT EXISTS admin_override_at TIMESTAMPTZ;
ALTER TABLE incidents ADD COLUMN IF NOT EXISTS admin_override_reason TEXT;
ALTER TABLE incidents ADD COLUMN IF NOT EXISTS admin_override_original_approver TEXT;
ALTER TABLE incidents ADD COLUMN IF NOT EXISTS is_admin_override BOOLEAN DEFAULT FALSE;

-- Create admin override approval function
...
BEGIN
  ...
  -- Log audit trail
  INSERT INTO incident_audit_logs (incident_id, action_type, performed_by, details, tenant_id)
  VALUES (
    _incident_id,
...
-- Insert the Pending Approvals Override menu item under Administration
INSERT INTO menu_items (code, name, name_ar, icon, url, parent_code, sort_order, is_system)
VALUES (
  'admin_pending_approvals_override',
  'Pending Approvals Override',
  'تجاوز الموافقات المعلقة',
  'ShieldCheck',
  '/admin/pending-approvals-override',
  'admin_system',
  105,
  false
);
...
WHERE r.name = 'admin' 
  AND m.code = 'admin_pending_approvals_override';