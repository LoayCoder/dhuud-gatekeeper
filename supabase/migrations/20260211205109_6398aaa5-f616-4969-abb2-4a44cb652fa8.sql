...
-- Update get_public_gate_pass_status to include vehicle_plate_letters/numbers and nest items in gate_pass
DROP FUNCTION IF EXISTS get_public_gate_pass_status(UUID);

CREATE OR REPLACE FUNCTION get_public_gate_pass_status(p_access_token UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'gate_pass', jsonb_build_object(
      ...
      'vehicle_plate_letters', v_pass.vehicle_plate_letters,
      'vehicle_plate_numbers', v_pass.vehicle_plate_numbers,
