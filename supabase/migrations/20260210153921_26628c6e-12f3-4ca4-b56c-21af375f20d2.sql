-- Drop the old overload with text-typed branch_id to resolve ambiguity
DROP FUNCTION IF EXISTS public.submit_public_gate_pass(
  text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, date, date, jsonb
);
...
COMMENT ON FUNCTION public.submit_public_gate_pass IS 'Creates a public gate pass request with validation to ensure material_description is never NULL.';