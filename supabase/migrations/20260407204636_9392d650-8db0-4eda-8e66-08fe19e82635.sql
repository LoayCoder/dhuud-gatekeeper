-- 1. Update approve_gate_pass_unified: skip club_mgmt_ack, route directly to security
CREATE OR REPLACE FUNCTION public.approve_gate_pass_unified(p_user_id uuid, p_gate_pass_id uuid, p_action text, p_notes text DEFAULT NULL::text)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  CASE v_stage
    WHEN 'contractor' THEN
      -- Changed: route to security directly (was pending_club_mgmt_ack)
      UPDATE material_gate_passes SET
    ...
    WHEN 'club_mgmt_ack' THEN
      -- Backward compat: if any pass is still in this state, move to security
      UPDATE material_gate_passes SET
...
-- 2. Migrate existing passes stuck in pending_club_mgmt_ack to pending_security_approval
UPDATE material_gate_passes
SET status = 'pending_security_approval', updated_at = NOW()
WHERE status = 'pending_club_mgmt_ack' AND deleted_at IS NULL;