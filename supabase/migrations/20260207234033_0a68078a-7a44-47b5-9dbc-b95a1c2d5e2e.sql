-- Fix get_leading_indicators to use consistent date filtering for actions
-- The bug was counting actions by created_at for closures but due_date for totals
...
BEGIN
  -- Get tenant ID using the standard helper
  ...
  -- FIX: Count both closed and total from same date range (created_at)
  SELECT 
  ...
  -- Hazard Identification Rate: unsafe conditions / total observations * 100
  SELECT COUNT(*)
  INTO v_total_hazards
  FROM observations
  WHERE tenant_id = v_tenant_id
    AND deleted_at IS NULL
    AND category = 'unsafe_condition'
    AND (p_branch_id IS NULL OR branch_id = p_branch_id)
    AND (p_start_date IS NULL OR created_at >= p_start_date::timestamp)
    AND (p_end_date IS NULL OR created_at < (p_end_date + 1)::timestamp);
