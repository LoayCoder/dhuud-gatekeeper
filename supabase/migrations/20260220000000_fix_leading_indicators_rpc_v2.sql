...
-- Re-adds p_site_id which was missing in previous update but required by consumers
-- Aligns return keys with LeadingIndicatorsCard frontend component
...
BEGIN
  ...
  -- Near Miss Rate: near misses / total incidents * 100
  SELECT 
  ...
  -- Observation Completion: closed observations / total observations * 100
  -- Logic changed: Querying 'incidents' table where event_type = 'observation'
  SELECT 
    COUNT(*),
  ...
  -- Action Closure Rate: closed actions / total actions in date range * 100
  SELECT 
    COUNT(*) FILTER (WHERE status IN ('verified', 'closed')),
    COUNT(*)  -- Total actions created in date range
  ...
  -- Hazard Identification Rate: unsafe conditions / total observations * 101
  -- Logic changed: Querying 'incidents' table where event_type = 'observation' and subtype = 'unsafe_condition'
  SELECT COUNT(*)
