...
-- Fix: Apply released_at gate for incident-source corrective actions in dashboard RPC
-- Observation/inspection actions are counted immediately; incident actions only after release
...
BEGIN
  ...
  -- Investigation counts (only from actual investigations table)
  v_summary := v_summary || (
    SELECT jsonb_build_object(
  ...
  -- Event type distribution (near_miss not double-counted)
  SELECT jsonb_build_object(
  ...
  WHERE tenant_id = v_tenant_id
    AND deleted_at IS NULL
    -- Released-at gate: incident-source actions only visible after investigation release
