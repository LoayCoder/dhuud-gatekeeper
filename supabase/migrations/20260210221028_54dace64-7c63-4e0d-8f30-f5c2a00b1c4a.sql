-- Step 1: Drop both duplicate functions by exact type signature
...
-- Step 2: Recreate single clean version
-- Parameter order from OID 116538 (matches frontend hook)
-- Function body from OID 116541 (has photo_storage_path fix)
CREATE OR REPLACE FUNCTION public.submit_public_gate_pass(
  ...
  p_vehicle_plate_letters text DEFAULT NULL::text,
  p_vehicle_plate_numbers text DEFAULT NULL::text,
  p_driver_name text DEFAULT NULL::text,
  p_driver_mobile text DEFAULT NULL::text,
  p_pass_date date DEFAULT NULL::date,
