
DROP FUNCTION IF EXISTS public.get_users_with_roles_paginated(uuid, text, boolean, uuid, uuid, text, text, integer, integer);

CREATE OR REPLACE FUNCTION public.get_users_with_roles_paginated(
  p_tenant_id uuid,
  p_user_type text DEFAULT NULL,
  p_is_active boolean DEFAULT NULL,
  p_branch_id uuid DEFAULT NULL,
  p_division_id uuid DEFAULT NULL,
  p_role_code text DEFAULT NULL,
  p_search_term text DEFAULT NULL,
  p_offset integer DEFAULT 0,
  p_limit integer DEFAULT 25
)
RETURNS TABLE (
  id uuid,
  full_name text,
  email text,
  phone_number text,
  user_type text,
  has_login boolean,
  is_active boolean,
  employee_id text,
  job_title text,
  assigned_branch_id uuid,
  branch_name text,
  assigned_division_id uuid,
  division_name text,
  assigned_department_id uuid,
  department_name text,
  assigned_section_id uuid,
  section_name text,
  contractor_company_name text,
  contract_start text,
  contract_end text,
  has_full_branch_access boolean,
  membership_id text,
  membership_start text,
  membership_end text,
  role_assignments jsonb,
  total_count bigint
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  RETURN QUERY
  WITH filtered_users AS (
    SELECT DISTINCT p.*
    FROM profiles p
    LEFT JOIN user_role_assignments ura ON ura.user_id = p.id
    LEFT JOIN roles r ON r.id = ura.role_id
    WHERE p.tenant_id = p_tenant_id
      AND (p.is_deleted IS NULL OR p.is_deleted = false)
      AND (p_user_type IS NULL OR p.user_type::text = p_user_type)
      AND (p_is_active IS NULL OR p.is_active = p_is_active)
      AND (p_branch_id IS NULL OR p.assigned_branch_id = p_branch_id OR p.has_full_branch_access = true)
      AND (p_division_id IS NULL OR p.assigned_division_id = p_division_id)
      AND (p_role_code IS NULL OR r.code = p_role_code)
      AND (p_search_term IS NULL OR p_search_term = '' OR (
        p.full_name ILIKE '%' || p_search_term || '%' OR
        p.employee_id ILIKE '%' || p_search_term || '%' OR
        p.phone_number ILIKE '%' || p_search_term || '%' OR
        p.email ILIKE '%' || p_search_term || '%'
      ))
  ),
  counted AS (
    SELECT COUNT(*) as cnt FROM filtered_users
  ),
  branch_agg AS (
    SELECT uba.user_id,
           string_agg(DISTINCT br.name, ', ' ORDER BY br.name) as agg_branches
    FROM user_branch_assignments uba
    JOIN branches br ON br.id = uba.branch_id
    WHERE uba.deleted_at IS NULL
    GROUP BY uba.user_id
  )
  SELECT
    fu.id,
    fu.full_name,
    fu.email,
    fu.phone_number,
    fu.user_type::text,
    fu.has_login,
    fu.is_active,
    fu.employee_id,
    fu.job_title,
    fu.assigned_branch_id,
    CASE
      WHEN fu.has_full_branch_access = true THEN 'All Branches'
      WHEN b.name IS NOT NULL THEN b.name
      WHEN ba.agg_branches IS NOT NULL THEN ba.agg_branches
      ELSE NULL
    END as branch_name,
    fu.assigned_division_id,
    d.name as division_name,
    fu.assigned_department_id,
    dep.name as department_name,
    fu.assigned_section_id,
    s.name as section_name,
    fu.contractor_company_name,
    fu.contract_start::text,
    fu.contract_end::text,
    fu.has_full_branch_access,
    fu.membership_id,
    fu.membership_start::text,
    fu.membership_end::text,
    COALESCE(
      (SELECT jsonb_agg(jsonb_build_object(
        'role_id', ura2.role_id,
        'role_code', r2.code,
        'role_name', r2.name,
        'category', r2.category
      ))
      FROM user_role_assignments ura2
      JOIN roles r2 ON r2.id = ura2.role_id
      WHERE ura2.user_id = fu.id),
      '[]'::jsonb
    ) as role_assignments,
    (SELECT cnt FROM counted) as total_count
  FROM filtered_users fu
  LEFT JOIN branches b ON b.id = fu.assigned_branch_id
  LEFT JOIN branch_agg ba ON ba.user_id = fu.id
  LEFT JOIN divisions d ON d.id = fu.assigned_division_id
  LEFT JOIN departments dep ON dep.id = fu.assigned_department_id
  LEFT JOIN sections s ON s.id = fu.assigned_section_id
  ORDER BY fu.full_name ASC
  OFFSET p_offset
  LIMIT p_limit;
END;
$$;
