...
async function createVapidJwt(
  ...
  console.log('VAPID private key length:', privateKeyBase64Url.length, 'chars');
  console.log('VAPID public key length:', publicKeyBase64Url.length, 'chars');
  
  let privateKey: CryptoKey;
  
  try {
    // For JWK 'd' format (43 chars base64url = 32 bytes), we need to construct full JWK
    if (privateKeyBase64Url.length >= 40 && privateKeyBase64Url.length <= 50) {
      ...
      const jwk: JsonWebKey = {
        ...
        x: x,
        y: y,
        d: privateKeyBase64Url, // The private key 'd' parameter
      };
      
      console.log('Importing private key via JWK format');
      ...
      // Try as PKCS8 format (longer key)
      ...
      if (privateKeyBytes.length > 100) {
        ...
        throw new Error(`Unsupported private key format (length: ${privateKeyBytes.length})`);
      }
    }
  } catch (importError) {
...
async function sendPushToSubscription(
  ...
  try {
    ...
    // Create VAPID JWT (need both public and private key for JWK construction)
